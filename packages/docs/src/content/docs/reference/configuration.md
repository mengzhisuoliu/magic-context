...
| `memory.enabled` | boolean | `true` | Enable cross-session memory (default: true). Does not affect history embedding or semantic ctx_search over session history; set embedding.provider to 'off' to stop all embedding. |
